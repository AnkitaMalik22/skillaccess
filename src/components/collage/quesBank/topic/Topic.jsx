...
import { setCurrentTopic,getAllTopics } from "../../../../redux/collage/test/testSlice";
...
const Topic = () => {
  ...
  const getTotalQuestions = (topic) => {
    let total = 0;
  total = topic?.questions?.length + topic?.video?.length + topic?.compiler?.length + topic?.essay?.length + topic?.findAnswers?.length;
    ...
    <div className="w-[95%] mx-auto font-dmSans">
      ...
      <div className="flex flex-wrap gap-4 w-full justify-center bg-gray-100 px-3 py-6 rounded-3xl ">
        ...
        {filteredSections?.map((section, index) => {
            return (
              <div className=" w-[17rem] mb-4 bg-white rounded-2xl" key={index}  onClick={() => {
                dispatch(
                  setCurrentTopic({
                    topic: section
            ...
            >
                ...
                <div className="card-body">
                  ...
                  <div className="flex gap-2">
                    <Folder />
                    <p className="text-[#95ACFA] text-sm">{getTotalQuestions(section)} Files</p>
                  </div>
                </div>
              </div>
            );
