...
const CampusDriveCard = ({ drive }) => {
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6 hover:shadow-xl transition-all duration-300">
      <img
        src={drive.coverPhoto ? URL.createObjectURL(drive.coverPhoto) : 'https://via.placeholder.com/150'}
        alt="Cover"
      ...
      <Link to={`/company/pr/campus-drive/${drive?._id}/tests`} className="text-blued mt-4 inline-block hover:underline">
        View Details
      </Link>
    </div>
  );
