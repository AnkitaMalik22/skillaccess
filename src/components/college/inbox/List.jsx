...
} from "../../../redux/college/Inbox/inboxSlice";
...
const List = ({ show, inboxType, setInboxType }) => {
  ...
  const { total, totalSent } = useSelector((state) => state.collegeAuth?.mail);

  const email = useSelector((state) => state.collegeAuth?.user?.Email);
  ...
  const handleNav = async (i, el) => {
    ...
    navigate(
      `/college/inbox/mail?index=${i}&inboxType=${inboxType}&type=view&show=${show}&within=${searchParams.get(
        "within"
            ...
            <div className="mb-4 bg-white rounded-lg flex justify-between p-4 ">
              <div className="flex gap-4 items-center ">
                ...
                <p
                  ...
                  // onClick={() => navigate("/college/inbox/mail?type=view")}
