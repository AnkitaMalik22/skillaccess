...
const TestCard = (props) => {
  ...
  return (
    <div className="w-full  bg-[#0d9aac23] text-start font-bold text-black rounded-md p-4 font-dmSans shadow-md hover:shadow-lg transition-shadow duration-300">
      ...
      <p className="font-normal text-gray-500 text-sm line-clamp-1 mb-2 h-[28%]">
        {props.assessment?.test?.description?.length > 100
          ? `${props.assessment?.test?.description.substring(0, 100)}...`
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {props?.assessment?.test?.topics?.slice(0, 4).map((topic, index) => (
          <div
            key={index}
            className="overflow-hidden bg-white rounded-md shadow-sm hover:shadow-md transition-shadow duration-300"
          >
