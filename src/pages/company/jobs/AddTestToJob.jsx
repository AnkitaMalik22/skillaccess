...
import {
  addTestToJob,
  removeTestFromJob,
} from "../../../redux/company/test/thunks/job";
...
const AddTestToJob = () => {
  ...
  const beginner = useSelector(
    (state) => state.companyTest.assessments.beginner
  );
  const intermediate = useSelector(
    (state) => state.companyTest.assessments.intermediate
  );
  const advanced = useSelector(
    (state) => state.companyTest.assessments.advanced
  );
  const adaptive = useSelector(
    (state) => state.companyTest.assessments.adaptive
  );
    ...
    <div className="mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tests.map((test) => (
          <div
            key={test._id}
            className={`p-4 border rounded-md cursor-pointer shadow-md transition duration-200 hover:shadow-lg ${
              selectedTestId === test._id
                ? "bg-blue-50 border-blued"
  ...
  return (
    <div className="p-6 bg-white shadow-lg rounded-md max-w-full mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          className="bg-blued text-white font-semibold py-2 px-6 rounded-md hover:bg-secondary transition duration-200"
          onClick={handleSave}
